...
Returns model and loss history for plotting.
"""
import torch
import copy
...
# cuDNN has issues with RNN on some driver versions — native CUDA impl works fine
...
def train(model, train_loader, val_loader, epochs, lr, device, use_scheduler=True):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch + 1}/{epochs} — Loss: {loss:.4f} | Val F1: {f1:.4f} | Δ {improvement:+.4f}")
